...
本参考定义以真实 `develop` 为集成基线的团队任务编排协议。执行前必须读取 `workspace-layout.md`、`task-format.md`、`projects.md` 和 `commit.md`。
...
3. `develop/` 必须对应真实 `develop` 分支。不存在时创建或复用分支：
...
3. 已有任务分支时直接切换；新任务先切回真实 `develop`，再创建分支：
...
## 阶段 4：总体验收和合并 develop

1. 启动 review subagent，审查 `<task-id>` 相对 `develop` 的完整变化、授权路径、记录和测试覆盖。
2. 有 finding 时更新任务记录，并重新派给对应 implementation subagent；修复后重新集成和 review。
3. review 通过后更新状态和日志，在 `<task-id>` 分支提交最终记录，并确认工作区干净。
4. 切回真实 `develop`，使用 merge commit 保留任务边界：

```bash
git -C develop switch develop
git -C develop merge --no-ff <task-id>
```

5. merge conflict 在当前 merge 中解决；完成后确认分支为 `develop`、工作区干净且 `<task-id>` 已成为 `develop` 的祖先。
6. 不自动 push。默认保留项目 worktree 和任务分支，只在结果中列出可清理项；未经用户要求不删除。
